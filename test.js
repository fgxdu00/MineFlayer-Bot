...
// Создаем отдельный процесс для дочерней консоли
const childProcess = spawn('node', ['childConsole.js'], { stdio: ['pipe', 'pipe', 'pipe', 'ipc'] });

childProcess.on('message', (message) => {
    console.log(`Child process is ready: ${message}`);
    
    // Отправляем сообщение в дочерний процесс после того, как он готов
    childProcess.send('Hello from the main process!');
});

// Отправляем сообщение в дочерний процесс
childProcess.send('Message from the main process!');
...
// Обработчик события завершения дочернего процесса
